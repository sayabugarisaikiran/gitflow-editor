    ...
    return (
      ...
      `}
        >
            ...
            {shortcut && (
                <span className="text-[9px] text-slate-400 dark:text-slate-600 font-mono">{shortcut}</span>
            )}
        </button>
    );
}
...
function Separator() {
    return <div className="my-1 border-t border-slate-200 dark:border-slate-700/40" />;
}
...
export default function NodeContextMenu({ menu, onClose }: NodeContextMenuProps) {
    ...
    return (
        ...
        >
            <div
                className="absolute bg-white dark:bg-[#151b28] border border-slate-200 dark:border-slate-700/60 rounded-xl shadow-2xl shadow-black/10 dark:shadow-black/60 w-60 py-1.5 animate-context-menu"
            ...
            >
                ...
                <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-700/30">
                    <div className="flex items-center gap-2">
                        <div className={`w-3 h-3 rounded-full shrink-0 ${menu.isHead ? 'bg-gradient-to-br from-yellow-400 to-orange-500' : 'bg-gradient-to-br from-slate-400 to-slate-500 dark:from-slate-600 dark:to-slate-700'}`} />
                        <span className="text-[11px] font-mono text-slate-700 dark:text-slate-300 truncate">
                            {menu.commitHash.slice(0, 8)}
                        </span>
                    </div>
                    <p className="text-[10px] text-slate-500 dark:text-slate-500 truncate mt-0.5 ml-5">
                        {menu.commitMessage}
                    </p>
                </div>
                ...
                {otherBranches.length === 0 && menu.branches.length > 0 && menu.branches.includes(currentBranch) && (
                    <>
                        ...
                        <div className="px-3 py-1.5">
                            <span className="text-[9px] text-slate-400 dark:text-slate-600 italic">
